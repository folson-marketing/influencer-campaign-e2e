...
| 选项 | 适合 | 需要什么 |
|---|---|---|
| 飞书多维表格 | 用飞书办公的团队、需要看板 / 自动化 | 飞书 / Lark 相关工具，或浏览器 |
| Google Sheets | 多人协作、与海外同事或代理共享 | Google Drive / Sheets 相关工具 |
...
- 用户不愿意装，或目标平台找不到合适的 actor：用浏览器直接在各平台搜索（TikTok、Instagram、YouTube 等的搜索页、话题页、竞品评论区），手动读取主页数据。告诉用户浏览器方式更慢，每次建议 20–30 个候选。
- 如果还有其他红人数据工具（如 Bizkol、Modash、HypeAuditor），问用户是否一并使用。

## 邮件

支持 **Gmail** 和 **Outlook / Microsoft 365**。检查对应工具，缺了就按上面的方式推荐安装；用户不装，或使用其他邮箱（如企业邮箱），就只输出文案让用户自己发。
